 ...
 */
export function applyTextDiff(oldText: string, diff: TextDiff): string | null {
  ...
  try {
    ...
    const sortedPatches = [...diff.patches].sort((a, b) => a.index - b.index);
    ...
    for (const patch of sortedPatches) {
      ...
      switch (patch.type) {
        case 'add':
          ...
          result.splice(adjustedIndex, 0, ...patch.value);
          offset += patch.value.length;
          break;

        case 'remove': {
          const length = patch.length || patch.value.length;
