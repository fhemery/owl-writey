...
Given('I am on the owl-writey homepage', async ({ homePo }: AllFixtures) => {
    await homePo.goTo(); 
});

// Exercise process
When ('I log as a known user for creating an exercise', async ({ loginPo, dashboardPo, exerciseCreatePo } : AllFixtures) => {
    await loginPo.goTo();
...
Then('I can try to take a turn on it, submit, cancel my turn, finish the exercise', async ({ exerciseCurrentPo, exquisiteCorpsePo, confirmDialogPo } : AllFixtures) => {
...
Then('I finally delete the exercise', async ({ exerciseCurrentPo, confirmDialogPo, dashboardPo, exerciseCardPo } : AllFixtures) => {
    await dashboardPo.shouldBeDisplayed();
...
// Novel process
When('I register as a new user', async ({ registerPo, dashboardPo } : AllFixtures) => {
...
Then('I can create, update a novel', async ({ novelCreatePo, novelCurrentPo, novelHeaderPo } : AllFixtures) => {
...
Then('I can delete the current novel', async ({ novelHeaderPo, novelCreatePo, confirmDialogPo } : AllFixtures) => {
    await novelHeaderPo.updateInfo();
