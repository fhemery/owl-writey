...
test.describe('API exercises', () => {
    ...
    // test('should join an exercise as a participant', async ({ request }) => {
    //     const exerciseData = {
    //         "name": "Invite to an exercise API test V2",
    //         "type": "ExquisiteCorpse",
    //         "config": {
    //             "nbIterations": 4,
    //             "iterationDuration": 300,
    ...
    //     // 1. On récupère un ID d'exercice
    //     const listResponse = await request.get(`${BASE_API_URL}/api/exercises`, {
    //         headers: { 'Authorization': `Bearer ${idToken}` }
    //     });
    //     const listResult = await listResponse.json();
    //     const exercisesList = listResult.exercises as Exercise[];
        
    //     // On cherche l'exercice qu'on vient de créer dans la liste (par son nom)
    //     const myExercise = exercisesList.find(ex => ex.name === exerciseData.name);

    //     expect(myExercise).toBeDefined();

    //     console.log("Nombre de participants à la création :", myExercise!.participants?.length);
    //     const exerciseId = myExercise!.id;

    //     // 2. On rejoint l'exercice
    //     const joinResponse = await request.post(`${BASE_API_URL}/api/exercises/${exerciseId}/participants`, {
    //         headers: {
    //             'Authorization': `Bearer ${idToken}`,
    //             'Accept': 'application/json'
    //         },
    //         // Si l'API attend un objet vide, on met {}, sinon on peut omettre 'data'
    //         data: {} 
    //     });
    ...
    //         headers: { 'Authorization': `Bearer ${idToken}` }
    //     });
    //     const exerciseDetail = await verifyResponse.json();

    //     expect(exerciseDetail.participants).toBeDefined();
    // });
});